...
- GUI 采用配置驱动：
  ...
  - 支持 `IconFunction: auto_claim` 自动领取开关按钮
...
自动领取按钮使用 `IconFunction: auto_claim`，开关是玩家个人状态，会保存到 `playerdata.yml` 或 MySQL。开启后插件每分钟检查一次在线玩家可领取奖励，达到时间会自动领取；玩家仍然可以手动点击奖励领取。
...
gui:
  ...
  GuiKey:
    ...
    "A":
      Material: "LIME_DYE"
      Name: "&a自动领取 &8» &f{auto_claim_status}"
      Lore:
        - "&7状态: {auto_claim_status_color}{auto_claim_status}"
        - "&e{auto_claim_toggle}"
      IconFunction: "auto_claim"
